...
module.exports = {
  ...
  examples: [
   ...
   execute(client, message, links){list.findById(message.guild.id, async (err, doc) => {
    ...
    .then(() => message.channel.send(
      ...
      .addFields(res.splice(0,25).sort((A,B) => B.id - A.id).map(entry => {
        ...
        if (tbd.includes(entry.id)){
          value = `<a:animatedcheck:815617149795565618> Successfully removed [**${mediatitle}**](${entry.siteUrl})`;
        } else {
          value = [
