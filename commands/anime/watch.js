...
module.exports = {
  ...
  examples: [
   ...
   execute(client, message, links){
    list.findById(message.guild.id, async (err, doc) => {
        ...
        .then(() => message.channel.send(
          new MessageEmbed()
            ...
            .addFields(res.splice(0, 25).sort((A, B) => B.id - A.id).map(entry => {
              ...
              return {
                name: '\u200b', value: [
                  ['\\⚠️ Failed to add', '<a:animatedcheck:815617149795565618> Successfully added'][Number(filter)],
