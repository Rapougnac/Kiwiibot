...
module.exports = {
	...
	async execute(client, message, args) {
		...
		const embedserv = new Discord.MessageEmbed()
			...
			.addField(`Rôles [${client.guild.roles.cache.size - 1}]`, client.guild.roles.cache.filter(r => r.id !== message.guild.id).sort((A,B) => B.rawPosition - A.rawPosition).map(x => `${x}`).splice(0,50).join(' | ') || '\u200b')
