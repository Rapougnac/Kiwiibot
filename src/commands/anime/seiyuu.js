  ...
  async execute(client, message, args) {
    ...
    try {
      ...
      if (res.errors && res.errors.some((e) => e.message !== 'Not Found.')) {
        ...
        return (
          (await mainpage.edit(embed).catch(() => null)) ||
      ...
      embed
        ...
        .setAuthor(
          [res.data.Staff.name.full, res.data.Staff.name.native]
            .filter(Boolean)
            .join('\u2000•\u2000'),
          null,
          res.data.Staff.siteUrl
        )
        .setDescription(
          [
            client.anischedule.info.langflags.find(
              (f) =>
                f.lang.toLowerCase() === res.data.Staff.language?.toLowerCase()
            )?.flag,
            text.truncate(
              toMarkdown(decode(res.data.Staff.description || '\u200b')),
              1000,
              `...[Read More](${res.data.Staff.siteUrl})`
            ),
          ].join('\n')
        )
        .addFields([
          {
            ...
            value:
              text.joinArrayAndLimit(
                res.data.Staff.characters.nodes.map((x) => {
                  return `[${x.name.full}](${x.siteUrl
                    .split('/')
                    .slice(0, 5)
                    .join('/')})`;
                }),
          ...
          },
          {
            ...
            value:
              text.joinArrayAndLimit(
                res.data.Staff.staffMedia.nodes.map((s) => {
                  return `[${s.title.romaji}](${s.siteUrl
                    .split('/')
                    .slice(0, 5)
                    .join('/')})`;
                }),
