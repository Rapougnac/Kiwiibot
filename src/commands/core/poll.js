...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!question)
      return message.channel.send('Veuillez indiquer la question à poser.');
    if (!choices.length)
      return message.channel.send('Veuillez indiquer au moins 1 choix.');
    if (choices.length > 20)
      return message.channel.send('Il ne peut pas y avoir plus de 20 choix.');
    message.delete();
