...
module.exports = {
  ...
  async execute(client, message, args) {
    if (!args[0]) {
      return message.channel.send(message.guild.i18n.__mf("common.missing_id"));
    }
    const mutedRole =
    ...
    GuildSchema.findOne({ _id: message.guild.id }, async (err, data) => {
      ...
      if (data) {
        ...
        await GuildSchema.findOneAndUpdate(
          ...
          return message.channel.send(message.guild.i18n.__mf("common.database_error",{error: err.name}));
        }
      } else {
